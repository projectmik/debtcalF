...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const addMasterExpenseField = (name = '', amount = 0, category = 'need') => {
        ...
        newExpense.innerHTML = `<input type="text" class="expense-name input-highlight w-full px-4 py-2 border rounded-md" placeholder="Item Name" value="${name}"><input type="number" class="expense-amount input-highlight w-32 px-4 py-2 border rounded-md" value="${amount}"><select class="expense-category input-highlight w-32 px-4 py-2 border rounded-md bg-white"><option value="need" ${category === 'need' ? 'selected' : ''}>Need</option><option value="want" ${category === 'want' ? 'selected' : ''}>Want</option><option value="saving" ${category === 'saving' ? 'selected' : ''}>Savings</option></select><button class="delete-item bg-red-100 text-red-600 px-2 py-2 rounded-md hover:bg-red-200">✕</button>`;
    ...
    const calculateAndDisplayBudget = (masterExpenses, debtResults) => {
        ...
        const savingsTotal = masterExpenses.filter(e => e.category === 'saving').reduce((sum, e) => sum + e.amount, 0);
        const debtTotal = (debtResults?.totalMinPayment || 0) + (debtResults?.extraPayment || 0);
        ...
        document.getElementById('budget-savings').value = savingsTotal.toFixed(2);
        document.getElementById('budget-debt').value = debtTotal.toFixed(2);
        updateBudgetChart(needsTotal, wantsTotal, savingsTotal, debtTotal, income);
    };
    
    const updateBudgetChart = (needs, wants, savings, debt, income) => {
        const ctx = document.getElementById('budgetChart')?.getContext('2d');
        if (!ctx) return;
        const allocated = needs + wants + savings + debt;
        const remaining = income - allocated;
        const data = {
            labels: ['Needs', 'Wants', 'Savings', 'Debt'],
            datasets: [{ 
                data: [needs, wants, savings, debt], 
                backgroundColor: ['#3b82f6', '#f97316', '#16a34a', '#6b21a8'],
                borderWidth: 2 
            }]
        };
        if (budgetChartInstance) {
            budgetChartInstance.data = data;
            budgetChartInstance.options.plugins.tooltip.callbacks.label = function(context) {
            ...
            budgetChartInstance = new Chart(ctx, { type: 'doughnut', data: data, options: { responsive: true, maintainAspectRatio: false, cutout: '60%', plugins: { legend: { position: 'bottom' }, tooltip: { callbacks: { 
    ...
    const calculateEmergencyFund = (masterExpenses) => {
        ...
        const savingsFromBudget = masterExpenses.filter(e => e.category === 'saving').reduce((sum, e) => sum + e.amount, 0);
        ...
        if (savingsFromBudget > 0 && (goal3Month - currentSavings) > 0) {
            const remainingToSave = goal3Month - currentSavings;
            const monthsToGoal = Math.ceil(remainingToSave / savingsFromBudget);
            timelineTextEl.textContent = `At ${formatCurrency(savingsFromBudget)}/month, you'll reach your goal in ${monthsToGoal} months.`;
    ...
    const calculateDebtFreedomPlan = (income, masterExpenses) => {
        ...
        const monthlySavings = masterExpenses.filter(e => e.category === 'saving').reduce((sum, e) => sum + e.amount, 0);
        let extraPayment;
    ...
    const displayDebtResults = (results) => {
        ...
        debtProgressChartInstance = new Chart(debtCtx, { type: 'line', data: { labels: Array.from({ length: results.monthsToFreedom + 1 }, (_, i) => `Month ${i}`), datasets: [{ label: 'Remaining Debt Balance', data: results.debtProgress, borderColor: '#2563eb', backgroundColor: 'rgba(37, 99, 235, 0.1)', fill: true, tension: 0.1 }] }, options: { responsive: true, plugins: { title: { display: true, text: 'Debt Balance Over Time' } }, scales: { y: { beginAtZero: true } } } });
    ...
    const calculateAndDisplayDebtResults = (income, masterExpenses) => {
        ...
        const zeroDebtState = document.getElementById('zero-debt-state');
        resultsContainer.classList.add('hidden');
        emptyState.classList.add('hidden');
        zeroDebtState.classList.add('hidden');
        if (results && results.totalDebt > 0) {
            displayDebtResults(results);
        } else if (results) { 
            const debtItems = document.querySelectorAll('#pillar3 .debt-item');
            const isPristine = debtItems.length === 1 && debtItems[0].querySelector('.debt-name').value === '' && (debtItems[0].querySelector('.debt-balance').value === '0' || debtItems[0].querySelector('.debt-balance').value === '');
            if (isPristine) {
                emptyState.classList.remove('hidden');
            } else {
                zeroDebtState.classList.remove('hidden');
            }
        }
    };
    ...
    const updateAllCalculators = () => {
        ...
        calculateAndDisplayDebtResults(income, masterExpenses);
    };
    ...
    if (document.getElementById('pillar3')) {
        document.querySelectorAll('#extra-payment, #target-months').forEach(input => {
            input.addEventListener('input', updateAllCalculators);
            attachNumberInputListeners(input);
        });
